...
/* eslint-disable */
/**
 * LEGACY ROUTE ADAPTER — DO NOT EXTEND.
 *
 * This is the pre-transformation backend (server.js @ 246c9d4), mechanically
 * converted from a standalone Express app into a Router mounted at /api.
 *
 * It exists for exactly one reason: app builds that are already on users' phones
 * point at these paths and cannot be updated retroactively. Removing them would
 * break every installed copy of both apps.
 *
 * What was changed in the conversion, and nothing else:
 *   - `app` became a Router; the `/api` prefix moved to the mount point
 *   - global middleware (json, cors, content-type) removed — now applied once in app.js
 *   - the hardcoded Neon connection string removed; uses the shared pool from src/db
 *   - the hardcoded MSG91 key removed; read from config
 *   - Socket.IO server creation removed; emits through the shared gateway
 *   - the two setInterval sweepers removed; they run once per deployment in src/jobs
 *   - server.listen removed
 *   - the local error handler removed — now global, and no longer leaks exception text
 *
 * Business logic is untouched, including its defects, because changing behaviour here
 * would change what shipped clients observe. Fixes land in /api/v1.
 *
 * Removal is gated on `deprecated_endpoint_usage` reaching zero, not on a date.
 */
...
// The shared pool. `pool.query` and `pool.connect` keep the original call sites working.
const pool = db.getPool();

// Emit shim: the original called io.to(room).emit(event, payload) directly.
const io = {
  to(room) {
    return {
      emit(event, payload) {
        const realIo = gateway.getIo();
        if (realIo) realIo.to(room).emit(event, payload);
      },
    };
  },
};

// Secrets, formerly hardcoded in this file.
const MSG91_AUTHKEY = config.otp.msg91.authKey;
const WA_TEMPLATE_NAME = config.otp.msg91.templateName;
const WA_NAMESPACE = config.otp.msg91.namespace;
const WA_INTEGRATED_NUMBER = config.otp.msg91.integratedNumber;

// The original kept OTPs in process memory on app.locals.
const otpStoreHolder = { otpStore: {} };
const app = { locals: otpStoreHolder };
...
router.post('/owner/bookings/verify', async (req, res) => {
  try {
...
router.get('/users/profile/:phone', async (req, res) => {
  try {
...
router.get('/users/bookings/:phone', async (req, res) => {
  try {
    const phone = req.params.phone;
    if (!phone) return res.status(400).json({ message: 'phone is required' });
...
// -------------------- Parking areas --------------------
router.get('/parking_areas', async (req, res) => {
  try {
...
router.get('/parking_areas/:id', async (req, res) => {
  try {
...
router.get('/parking_areas/:id/pricing', async (req, res) => {
  try {
...
router.get('/parking_areas/:id/slots', async (req, res) => {
  try {
...
router.post('/bookings', processBooking);
router.post('/owner/bookings', processBooking);
...
router.get('/owner/all', async (req, res) => {
  try {
...
router.post('/owner/parking_areas', async (req, res) => {
  const {
    name: ownerPhone,
    parking_area_name,
    location,
...
router.get('/owner/parking_areas', async (req, res) => {
  try {
    const areas = await pool.query('SELECT * FROM parking_areas');
    const withLocation = areas.rows.map(a => ({ ...a, location: { lat: a.lat, lng: a.lng } }));
    return res.status(200).json(withLocation);
...
router.get('/owner/parking_areas/:id/slots', async (req, res) => {
  try {
    const parking_id = toInt(req.params.id);
    if (!parking_id) return res.status(400).json({ message: 'Invalid parking area ID' });
...
router.post('/owner/bookings/complete', async (req, res) => {
  const client = await pool.connect();
  try {
...
router.post('/dev/reset-active-state', async (req, res) => {
